# Sealed-salt ceremony (Level 2)

> Public protocol for the generation and split of the secret salt used in Level 2.

## Why this exists

The Level 1 design (public salt only) is vulnerable to any adversary who obtains the official list of Andorran NIAs: they can deanonymise the petition in ~4 CPU-hours. To raise the bar to "infeasible", Level 2 adds a 32-byte **secret salt** that is split between five public custodians using Shamir Secret Sharing with threshold 3.

While the petition is being collected:
- The salt is **never** held in full by any single person.
- Three of the five custodians must cooperate to reconstitute it.
- Each custodian has publicly committed not to do so until the petition concludes.

This protects against political pressure on any single actor — government, party, journalist, organiser. No single party can deanonymise the petition.

## Who are the five custodians

To be selected and publicly named before launch. Plausible candidates:

1. **Artur Homs**, organiser of the petition, on behalf of Claror.
2. An Andorran public notary, acting in their professional capacity.
3. A representative of the Col·legi d'Advocats d'Andorra.
4. A representative of the Universitat d'Andorra (faculty member, ideally in law or computer science).
5. A senior journalist from one of the independent Andorran outlets (ARA, Diari d'Andorra, Bondia, Alto).

Each custodian must sign the [public commitment text](#public-commitment-text) below before the ceremony.

## Ceremony protocol

The ceremony is **filmed in full**. The video is published on the ReferendumJa archive repository. The ceremony has 7 steps.
...
### 3. Split with Shamir 3-of-5

Using `ssss-split` from the `ssss` package (Debian-packaged, open source, audited):

```bash
xxd -p -c 1000 secret-salt.bin | ssss-split -t 3 -n 5 -x -q > shares.txt
```

The five shares are printed on separate sheets of paper, each with the custodian's name.

### 4. Hand off the shares

Each custodian receives:
...
- A signed receipt acknowledging their custodianship and the public commitment.

The five custodians physically sign each other's receipts so each holds proof of the others' participation.
...
- The five signed custodianship receipts.
- This protocol document, with a Git tag signed by all five custodians.

## Reconstitution (end of petition only)

When the petition concludes:
1. Three of the five custodians convene in person.
2. The ceremony is again filmed.
3. The shares are combined with `ssss-combine`:
   ```bash
   ssss-combine -t 3 -q < shares.txt | xxd -r -p > secret-salt.bin
...
Each custodian signs the following before the ceremony:

> I, [name and function], hereby accept to act as one of the five public custodians of the secret salt used by the ReferendumJa petition platform.
>
> I commit:
...
> - to immediately and publicly notify the organisation and the four other custodians of any attempt — by anyone, including a state actor — to obtain my share by coercion or otherwise;
